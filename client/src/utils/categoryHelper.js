export const cleanSubCategoryName = (sub = '') => {
  if (!sub) return '';
  return sub.replace(/[\u{1F000}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]/gu, '').trim();
};
...
export const getSubCategory = (item, categoryName = '') => {
  ...
  if (cat.includes('starter')) {
    if (item.subCategory && item.subCategory.trim()) return cleanSubCategoryName(item.subCategory);
  ...
  if (cat.includes('biryani') || (cat === '' && name.includes('biryani'))) {
    if (item.subCategory && item.subCategory.trim() && item.subCategory.toUpperCase().includes('BIRYANI')) {
      return cleanSubCategoryName(item.subCategory);
    }
  ...
  if (cat.includes('main') || cat.includes('course')) {
    if (item.subCategory && item.subCategory.trim() && item.subCategory.toUpperCase().includes('MAIN COURSE')) {
      return cleanSubCategoryName(item.subCategory);
    }
    if (
