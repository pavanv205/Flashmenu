...
import { Zap, Mail, Lock, ArrowRight, KeyRound } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleQuickFill = (targetEmail, targetPassword = 'password123') => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F17] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-dark-card border border-dark-border rounded-3xl p-8 shadow-2xl relative z-10 space-y-6">
        ...
        <div className="p-4 bg-dark-base rounded-2xl border border-dark-border space-y-2">
          <div className="flex items-center space-x-1.5 text-amber-400 text-[11px] font-extrabold uppercase tracking-wider">
            <KeyRound className="w-3.5 h-3.5" />
            <span>Quick Fill Credentials (1-Tap Sign In)</span>
          </div>

          <div className="grid grid-cols-2 gap-2 pt-1">
            <button
              type="button"
              onClick={() => handleQuickFill('pavanvadapalli26@gmail.com', 'password123')}
              className="px-2.5 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-300 text-left transition-all group"
            >
              <div className="text-[10px] font-bold text-amber-400 group-hover:underline">Pavan Admin</div>
              <div className="text-[10px] text-gray-400 font-mono truncate">pavanvadapalli26@gmail.com</div>
            </button>

            <button
              type="button"
              onClick={() => handleQuickFill('demo@flashmenu.com', 'password123')}
              className="px-2.5 py-2 rounded-xl bg-gray-800/60 hover:bg-gray-800 border border-gray-700 text-gray-300 text-left transition-all group"
            >
              <div className="text-[10px] font-bold text-white group-hover:underline">Demo Restaurant</div>
              <div className="text-[10px] text-gray-400 font-mono truncate">demo@flashmenu.com</div>
            </button>
          </div>
        </div>
